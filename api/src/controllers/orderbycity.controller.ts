...
export const getOrderedCities: RequestHandler = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if(country) {
            ...
            return !orderedActivities.length? res.status(400).send(noactivities) : res.status(200).send(<ServerResponse>({status: 'success', message: 'Activities sucesfully loaded', data: orderedActivities}))
        ...
        return res.status(error.status || 400).json(<ServerResponse>({status: 'error', message: error.message || error}));
    }
